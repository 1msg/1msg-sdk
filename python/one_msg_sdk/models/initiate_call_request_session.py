...
class InitiateCallRequestSession(BaseModel):
    """
    WebRTC SDP session (required for connect / pre_accept / accept)
    """ # noqa: E501
    sdp_type: StrictStr = Field(description="offer for outbound connect; answer for pre_accept/accept")
    sdp: StrictStr = Field(description="Full SDP (RFC 8866). Replace placeholders with SDP from your WebRTC stack. Must negotiate ICE, DTLS-SRTP, and OPUS for WhatsApp media. ")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["sdp_type", "sdp"]

    @field_validator('sdp_type')
    def sdp_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['offer', 'answer']):
            raise ValueError("must be one of enum values ('offer', 'answer')")
        return value
