...
pub async fn get_me(configuration: &configuration::Configuration, token: &str) -> Result<models::ProfileInfo, Error<GetMeError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ProfileInfo`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ProfileInfo`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn update_me(configuration: &configuration::Configuration, token: &str, update_me_request: Option<models::UpdateMeRequest>) -> Result<std::collections::HashMap<String, serde_json::Value>, Error<UpdateMeError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_token = token;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<UpdateMeError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
