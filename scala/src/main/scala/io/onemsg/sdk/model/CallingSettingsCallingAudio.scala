...
import io.onemsg.sdk.core.JsonSupport._
   ...
   */
case class CallingSettingsCallingAudio(
  status: Option[String] = None
)
object CallingSettingsCallingAudio {
  implicit val encoderCallingSettingsCallingAudio: Encoder[CallingSettingsCallingAudio] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  implicit val decoderCallingSettingsCallingAudio: Decoder[CallingSettingsCallingAudio] = Decoder.instance { c =>
    for {
      status <- c.downField("status").as[Option[String]]
    } yield CallingSettingsCallingAudio(
      status = status
    )
  }
}
