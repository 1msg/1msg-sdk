...
import io.onemsg.sdk.core.JsonSupport._
   ...
   */
case class InitiateCallResponseResponse(
  error: Option[String] = None
)
object InitiateCallResponseResponse {
  implicit val encoderInitiateCallResponseResponse: Encoder[InitiateCallResponseResponse] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  implicit val decoderInitiateCallResponseResponse: Decoder[InitiateCallResponseResponse] = Decoder.instance { c =>
    for {
      error <- c.downField("error").as[Option[String]]
    } yield InitiateCallResponseResponse(
      error = error
    )
  }
}
